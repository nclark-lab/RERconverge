---
title: "| SUPPLEMENTARY INFORMATION \n| RERconverge: an R package for associating
  evolutionary rates with convergent traits \n"
...
* `useSpecies`: a vector that can be used to specify a subset of species to use in the analysis, or conversely used to exclude species from analyses (branches leading to these species will be set to NA). Here we will use the species in our AdultWeightLog vector that will be used for continuous trait analysis. Note that these are also the same species used for binary trait analysis.
...
In this case, we will start with the data from the continuous trait analysis described  [above](#ContinuousTrait) that used adult mass as the phenotype of interest in mammalian species.  This walkthrough assumes that you have already installed RERconverge.
