...
We now provide tools for users to estimate approximate maximum likelihood trees from nucleotide or amino acid alignments using the `pml` and `optim.pml` functions from the [phangorn](https://cran.r-project.org/web/packages/phangorn/index.html) package [@Schliep2011].
...
First, the code tells us that there are 500 items, or gene trees, in the file. Then it says that the maximum number of tips in the gene trees is 62 and, later, it reports that it will use the 32 genes in this set that have data for all 62 species to estimate a **master tree**. The master tree will be used for subsequent analyses.
...
We will use the `getAllResiduals` function to calculate RERs. The following list explains the input variables and their defaults (though there are other arguments in the documentation):
...
The first output of this function tells you that the cutoff is set to 3.2e-05. Any branches shorter than this will be excluded from the analysis. It then  calculates relative evolutionary rates for sets of gene trees.
...
```{r fig2d, fig.height=9,fig.width=10, warning = FALSE, cache = TRUE, tidy = FALSE}
...
## Continuous Trait Analysis {#ContinuousTrait}
...
repopath = '~/Documents/GitHub/RERconverge_testing'
...
In this case, we will start with the data from the continuous trait analysis described [above](#ContinuousTrait) that used adult mass as the phenotype of interest in mammalian species. This walkthrough assumes that you have already installed RERconverge, and it uses output from the *correlateWithContinuousPhenotype* function ("RERresults") that is included with the RERconverge package.
