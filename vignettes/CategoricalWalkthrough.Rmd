...
This function uses code from castor and internally calls the function
`getAncLiks` which borrows heavily from the function `ace` in ape and
from the functions `fitMk` and `rerootingMethod` in phytools. The
relevant citations are given below:
...
# unrooted
phenTree = char2TreeCategorical(sleepPattern, toyTrees, useSpecies = allspecs, model = "SYM",
                                plot = TRUE)
```
...
phenTree = char2TreeCategorical(sleepPattern, toyTrees, useSpecies = allspecs, plot = TRUE,
                                anctrait = "NOCTURNAL")
```
...
Some of the gene trees are missing species from the master tree. To
handle missing species, RERconverge generates paths. For a more detailed
...
# unrooted
charP = char2PathsCategorical(sleepPattern, toyTrees, useSpecies = allspecs, model = "ER", 
                              plot = FALSE)
```
...
```{r, eval=FALSE}
charP = tree2Paths(phenTree, trees, useSpecies = names(sleepPattern), categorical = TRUE)
```
...
root_state = 3 # DIURNAL (3) is the state at the root
states = c(root_state, phenTree$edge.length[order(phenTree$edge[,2])])
