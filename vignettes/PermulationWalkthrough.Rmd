...
# Calculate RERs
mamRERw = getAllResiduals(toyTrees, transform="sqrt", use.weights=T)
...
TTNtreeSSM = simBinPhenoSSM(TTNtree, toyTrees, marineFg, pathvec,
                            plotTreeBool=F)
...
ACBD5treeSSM = simBinPhenoSSM(ACBD5tree, toyTrees, marineFg,pathvec, plotTreeBool=F)
...
#calculate RERs
mamRERw = getAllResiduals(toyTrees,useSpecies=names(logAdultWeightcm), transform = "sqrt")
...
res$permpval=corpermpvals[match(rownames(res), rownames(corpermpvals)),]
res$permpvaladj=p.adjust(res$permpval$permpval, method="BH")
count=1
...
Permulation of categorical (including binary) phenotypes requires a rooted phylogenetic tree, observed phenotype values, a rate model (in this case "ER" stands for "Equal Rates", where we assume equal transition states among categorical trait states) and a target number of trees to permulate (in this case we use 1 as demonstration).
...
catperm =categoricalPermulations(toyTrees, marinephen, rm="ER", ntrees=1)
