...
marineFgTree2 = foreground2TreeClades(marineFg,sisters_marine2,toyTrees,plotTree=T)
```
...
sisters_Example = list("clade1"=c("Golden_hamster","Chinese_hamster"),"clade2"=c("clade1","Vole"))

exampleFgTree = foreground2TreeClades(fgExample,sisters_Example,toyTrees,plotTree=T)
```
...
res = correlateWithBinaryPhenotype(mamRERw, pathvec, min.sp=10, min.pos=2, weighted="auto")
```
...
permCC = getPermsBinary(100, marineFg, sisters_marine, root_sp, mamRERw, toyTrees, masterTree, permmode="cc")
...
treeCC = simBinPhenoCC(toyTrees, masterTree, root_sp, marineFg, sisters_marine, pathvec, plotTreeBool=T)
```

For SSM, the permulated tree of a gene may be different from that of another gene if they have different "species membership", meaning if the gene trees may be missing different sets of species. Below is an example permulation of a gene that has all marine foregrounds in its tree:
...
TTNtreePlot = plotTreeHighlightBranches(TTNtree, 
                                    hlspecies=marineFg, hlcols=rep("blue",length(marineFg)), 
                                    main="TTN tree") #plot individual gene tree
```

```{r, message=F, warning=F, error=F, results=F}
#generate and plot a SSM permulation of TTN
TTNtreeSSM = simBinPhenoSSM(TTNtree, toyTrees, root_sp, marineFg, sisters_marine, pathvec, plotTreeBool=T)
```
...
ACBD5treePlot = plotTreeHighlightBranches(ACBD5tree, 
                                    hlspecies=marineFgACBD5, hlcols=rep("blue",length(marineFgACBD5)), 
                                    main="ACBD5 tree")
...
ACBD5treeSSM = simBinPhenoSSM(ACBD5tree, toyTrees, root_sp, marineFg, sisters_marine, pathvec, plotTreeBool=T)
```
...
TTNpermulatedTreesCC = generatePermulatedBinPhen(masterTree, numperms, toyTrees, root_sp, marineFg, sisters_marine, pathvec, permmode="cc")
```
...
TTNpermulatedTreesSSM = generatePermulatedBinPhen(TTNtree, numperms, toyTrees, root_sp, marineFg, sisters_marine, pathvec, permmode="ssm")
```
...
SSMpermulatedTreesBatched = generatePermulatedBinPhenSSMBatched(trees_example, numperms, toyTrees, root_sp, marineFg, sisters_marine, pathvec)
```
